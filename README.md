...
Turnstile Pass therefore provides a default callback, `turnstilePassOnError`. `script()` defines it in an inline script placed just before the Turnstile script, and `widget()` points each widget at it. It ignores retryable codes and leaves them to Turnstile's automatic retry. The configuration codes listed below are rethrown as an uncaught `Error` whose message carries the code, once per code and page, so a configuration problem still reaches global error handlers and error-monitoring tools. Any other code, such as the visitor-side `200100` or a code Cloudflare does not list, only makes Turnstile log a console warning. It shows the visitor nothing.
